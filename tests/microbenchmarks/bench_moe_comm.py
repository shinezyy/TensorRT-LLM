...
def _run_benchmark_worker_under_current_mpi(args: argparse.Namespace) -> None:
    ...
    # Give nsys time to finalize and write the report before MPI processes exit.
    # Without this delay, the processes may exit too quickly, causing PMIX errors
    # and preventing nsys from writing the .nsys-rep file.
    torch.cuda.synchronize()
    mpi_barrier()
    import time
    time.sleep(2)  # Allow nsys to flush profiling data
...
def main() -> None:
    ...
    # Direct MPI mode: if launched with multiple MPI ranks (e.g., srun --ntasks-per-node N),
    # run the benchmark directly without spawning.
    if world_size > 1:
        if world_size != ep_size:
            raise RuntimeError(
                f"Direct MPI mode: mpi_world_size ({world_size}) must equal --ep_size ({ep_size}). "
                f"Use srun --ntasks-per-node {ep_size} or adjust --ep_size."
            )
        _run_benchmark_worker_under_current_mpi(args)
        return

    # Spawn mode: parent is NOT part of the worker MPI world; do not call mpi_barrier here.
