...
When ``AC9_BYTES_SIDECAR`` is set, the env-gated hook in
``tensorrt_llm/_torch/visual_gen/attention_backend/_ac9_bytes_sidecar.py``
installs itself automatically on module import and writes per-rank JSON
at process exit. The same hook is used by the plan-scale ``trtllm-serve
+ nsys`` path (see ``serve-nsys-job.sh``), so both paths produce
identically-shaped sidecars for ``ac9_audit.py --measured-bytes-sidecar``.
"""
...
def main(num_iters: int = 3) -> None:
    ...
    os.environ.setdefault("AC9_ULYSSES_SIZE", str(world_size))
    local_rank = int(os.environ.get("SLURM_LOCALID", str(rank % torch.cuda.device_count())))
    torch.cuda.set_device(local_rank)
    dist.init_process_group(backend="nccl", rank=rank, world_size=world_size)

    # The AC9 bytes-sidecar hook activates on import of
    # ``tensorrt_llm._torch.visual_gen.attention_backend.parallel``
    # when ``AC9_BYTES_SIDECAR`` is set in the process environment, so
    # there is no explicit install call here. See
    # ``attention_backend/_ac9_bytes_sidecar.py``.
